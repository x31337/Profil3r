...
class OsintUtilities:
    ...
    def _send_recon_request(self, url_template, username, results_list, platform_name):
        ...
        try:
            response = requests.get(
                ...
                timeout=self.config.get("recon_timeout", 10),
            )
            if response.status_code == 200:
                ...
                self.logger.debug(
                    f"'{username}' not found on {platform_name} (404): {url}"
                )
                results_list.append(
                    {
                        "platform": platform_name,
                ...
                self.logger.debug(
                    f"Possible presence or issue for '{username}' on {platform_name}: {url} (Status: {response.status_code})"
                )
                results_list.append(
                    {
                        "platform": platform_name,
            ...
            self.logger.warning(
                f"Timeout checking {platform_name} for '{username}': {url}"
            )
            results_list.append(
                {"platform": platform_name, "url": url, "status": "timeout"}
            )
        except requests.exceptions.RequestException as e:
            self.logger.warning(
                f"Request error checking {platform_name} for '{username}': {url} - {e}"
            )
            results_list.append(
                {
                    "platform": platform_name,
    ...
    def user_reconnaissance(self, username_to_check):
        ...
        results = []
        threads = []
        thread_delay = self.config.get(
            "recon_thread_delay", 0.1
        )  # Short delay between starting threads

        for platform_name, url_template in platforms.items():
            thread = Thread(
                target=self._send_recon_request,
                args=(url_template, username_to_check, results, platform_name),
            )
        ...
        for t in threads:
            t.join(
                timeout=self.config.get("recon_thread_join_timeout", 15)
            )  # Join with timeout

        self.logger.info(
            f"Username reconnaissance for '{username_to_check}' complete. Found on {sum(1 for r in results if r['status'] == 'found')} platforms."
        )
        return results
    ...
    def _check_single_email(self, email, api_key, results_list):
        ...
        if not api_key:
            self.logger.warning(
                "RealEmail API key not configured. Cannot validate email."
            )
            results_list.append(
                {
                    "email": email,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results_list.append(
                    {"email": email, "status": "checked", "provider_status": status}
                )
                if status == "valid":
                    return {
                        "email": email,
                        "status": "valid",
                    }  # Return valid email for immediate use
            else:
                self.logger.warning(
                    f"Error validating email '{email}': HTTP {response.status_code} - {response.text[:100]}"
                )
                results_list.append(
                    {
                        "email": email,
            ...
            self.logger.error(
                f"JSON decode error for email '{email}'. Response: {response.text[:200]}"
            )
            results_list.append(
                {"email": email, "status": "json_error", "provider_status": "unknown"}
            )
        return None
    ...
    def find_emails_by_name(self, full_name, email_providers=None, custom_api_key=None):
        ...
        if not api_key_to_use:
            self.logger.warning(
                "RealEmail API key is not configured. Email validation will be skipped."
            )
        ...
        if email_providers is None:
            email_providers = self.config.get(
                "default_email_providers_for_search",
        ...
        potential_emails = [
            f"{user_var}@{provider}"
            for user_var in extended_variations
            for provider in email_providers
        ]
        self.logger.info(
            f"Generated {len(potential_emails)} potential emails to check."
        )
        ...
        for i, email_addr in enumerate(potential_emails):
            if i % 10 == 0 and i > 0:  # Log progress
                self.logger.debug(
                    f"Checked {i}/{len(potential_emails)} potential emails."
                )

            thread = Thread(
                target=self._check_single_email,
                args=(email_addr, api_key_to_use, validation_results),
            )
            threads.append(thread)
            thread.start()
            if len(threads) >= max_threads:  # Manage thread pool
                for t in threads:
                    t.join(
                        timeout=self.config.get("email_check_thread_join_timeout", 15)
                    )
                threads = []
        ...
        valid_emails_found = [
            res for res in validation_results if res.get("provider_status") == "valid"
        ]
        self.logger.info(
            f"Email finding complete for '{full_name}'. Found {len(valid_emails_found)} valid emails out of {len(potential_emails)} checked."
        )
    ...
    def get_github_user_info(self, username):
        ...
        try:
            response = requests.get(
                url, headers=headers, timeout=self.config.get("github_api_timeout", 10)
            )
            ...
            if http_err.response.status_code == 404:
                ...
                self.logger.error(
                    f"GitHub API HTTP error for '{username}': {http_err} - {response.text[:200]}"
                )
        except requests.exceptions.RequestException as req_err:
            self.logger.error(f"GitHub API request error for '{username}': {req_err}")
        except json.JSONDecodeError:
            self.logger.error(
                f"Failed to decode JSON from GitHub API for '{username}'."
            )
        return None
    ...
    def generate_temporary_email(self, custom_name=None):
        ...
        domain_choices = self.config.get(
            "temp_mail_domains", ["1secmail.com", "1secmail.net", "1secmail.org"]
        )
        ...
        check_url = (
            f"{TEMPMAIL_API_URL}?action=getMessages&login={login}&domain={domain}"
        )
        try:
            response = requests.get(
                check_url, timeout=self.config.get("temp_mail_api_timeout", 5)
            )
            if (
                response.status_code == 200
            ):  # Even an empty list is a success for creation
                self.logger.info(f"Temporary email generated: {temp_email}")
                return {"email": temp_email, "login": login, "domain": domain}
            else:
                self.logger.error(
                    f"Failed to verify temporary email generation (HTTP {response.status_code}): {temp_email}"
                )
        except requests.RequestException as e:
            self.logger.error(
                f"Error during temporary email generation for {temp_email}: {e}"
            )
        return None
    ...
    def get_temporary_emails(self, temp_email_info, last_id_checked=0):
        ...
        """
        if not temp_email_info or not all(
            k in temp_email_info for k in ["login", "domain"]
        ):
        ...
        try:
            response = requests.get(
                url, timeout=self.config.get("temp_mail_api_timeout", 10)
            )
            ...
            for msg_summary in raw_messages:
                if msg_summary.get("id", 0) > last_id_checked:
                    ...
                    try:
                        msg_detail_res = requests.get(
                            read_url,
                            timeout=self.config.get("temp_mail_api_timeout", 5),
                        )
                        msg_detail_res.raise_for_status()
                        messages.append(msg_detail_res.json())
                    except Exception as e_detail:
                        self.logger.warning(
                            f"Could not fetch detail for email ID {msg_id}: {e_detail}"
                        )
            self.logger.info(
                f"Fetched {len(messages)} new emails for {login}@{domain}."
            )
        except requests.RequestException as e:
            self.logger.error(f"Error fetching temp emails for {login}@{domain}: {e}")
        except json.JSONDecodeError:
            self.logger.error(
                f"JSON decode error fetching temp emails for {login}@{domain}."
            )
        return messages
...
if __name__ == "__main__":
    ...
    if utils.realemail_api_key:
        ...
        email_find_results = utils.find_emails_by_name(
            test_name, email_providers=["gmail.com", "tesla.com", "spacex.com"]
        )
        ...
        for res in email_find_results:
            if res.get("provider_status") == "valid":
                ...
                res.get("provider_status") != "unknown"
                and res.get("provider_status") != "api_key_missing"
            ):  # Log other statuses too
                test_logger.info(
                    f"  {res.get('provider_status', 'error').capitalize()}: {res['email']}"
                )
        test_logger.info(f"Total valid emails found for '{test_name}': {valid_emails}")
    else:
        test_logger.warning(
            "Skipping Email Finder test as REALEMAIL_API_KEY is not set."
        )
    ...
    if github_info:
        test_logger.info(
            f"GitHub info for '{test_github_user}': Name - {github_info.get('name')}, Public Repos - {github_info.get('public_repos')}"
        )
    else:
