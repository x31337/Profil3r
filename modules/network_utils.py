...
class NetworkUtilities:
    ...
    def get_ip_location(self, ip_address=None):
        ...
        if not ip_address:
            try:
                ...
                self.logger.warning(f"Could not auto-detect public IP: {e_pub_ip}. Trying curl fallback.")
                # Fallback to curl if requests method fails (as in original E4GL30S1NT)
                try:
                    process = subprocess.run(["curl", "-s", "ifconfig.co"], capture_output=True, text=True, check=True, timeout=5)
                    ip_address = process.stdout.strip()
                    self.logger.info(f"Detected public IP (via curl): {ip_address}")
                except Exception as e_curl:
                    self.logger.error(f"Error getting local public IP using curl: {e_curl}")
        ...
        try:
            response = requests.get(IPINFO_API_URL.format(ip_address), headers=DEFAULT_HEADERS, timeout=self.config.get("ipinfo_timeout", 10))
            response.raise_for_status()
            location_data = response.json()
            self.logger.info(f"Location data for {ip_address}: {location_data.get('city')}, {location_data.get('country')}")
            return location_data
        except requests.exceptions.HTTPError as http_err:
            self.logger.error(f"IPInfo API HTTP error for {ip_address}: {http_err} - {response.text[:200]}")
            return {"error": f"API error: {http_err.response.status_code}", "details": response.text[:200]}
        except requests.exceptions.RequestException as req_err:
            self.logger.error(f"IPInfo API request error for {ip_address}: {req_err}")
            return {"error": f"Request error: {req_err}"}
        except json.JSONDecodeError:
            self.logger.error(f"Failed to decode JSON from IPInfo API for {ip_address}.")
    ...
    def get_domain_info_hackertarget(self, target_domain_or_ip, scan_type):
        ...
        if not target_domain_or_ip:
            self.logger.error("Target domain or IP cannot be empty for HackerTarget scan.")
            return None
        ...
        self.logger.info(f"Requesting '{scan_type}' for '{target_domain_or_ip}' from HackerTarget.")
        url = API_HACKERTARGET.format(scan_type, target_domain_or_ip)
        try:
            response = requests.get(url, headers=DEFAULT_HEADERS, timeout=self.config.get("hackertarget_timeout", 20)) # Increased timeout
            response.raise_for_status()
            # HackerTarget API often returns plain text
            self.logger.info(f"Successfully fetched '{scan_type}' data for '{target_domain_or_ip}'.")
            return response.text
        except requests.exceptions.HTTPError as http_err:
            self.logger.error(f"HackerTarget API HTTP error for {scan_type} on {target_domain_or_ip}: {http_err} - {response.text[:200]}")
        except requests.exceptions.RequestException as req_err:
            self.logger.error(f"HackerTarget API request error for {scan_type} on {target_domain_or_ip}: {req_err}")
        return None
    ...
    def get_phone_info(self, phone_number, api_key=None):
        ...
        try:
            response = requests.get(api_url, headers=DEFAULT_HEADERS, timeout=self.config.get("veriphone_timeout", 10))
            response.raise_for_status()
            data = response.json()
            self.logger.info(f"Successfully fetched phone info for {phone_number}.")
            return data
        except requests.exceptions.HTTPError as http_err:
            self.logger.error(f"Veriphone API HTTP error for {phone_number}: {http_err} - {response.text[:200]}")
            return {"error": f"API error: {http_err.response.status_code}", "details": response.text[:200]}
        except requests.exceptions.RequestException as req_err:
            self.logger.error(f"Veriphone API request error for {phone_number}: {req_err}")
            return {"error": f"Request error: {req_err}"}
        except json.JSONDecodeError:
            self.logger.error(f"Failed to decode JSON from Veriphone API for {phone_number}.")
            return {"error": "JSON decode error"}

    def google_dork_search(self, dork_query, num_results=10):
        """Performs Google dorking and returns result URLs and titles."""
        if not google_search:
            self.logger.error("Python library 'google' is not installed. Cannot perform Google Dork search.")
        ...
        self.logger.info(f"Performing Google dork search for: '{dork_query}' (max {num_results} results)")
        results = []
        try:
            # The 'googlesearch' library handles iteration internally.
            # It might make multiple HTTP requests, so can be slow or hit limits.
            for url in google_search(dork_query, num_results=num_results, sleep_interval=2, timeout=5): # Added sleep_interval
                title = "N/A"
                try: # Try to fetch title, but don't let it break the dorking
                    page_response = requests.get(url, headers=DEFAULT_HEADERS, timeout=5)
                    ...
                    if title_element:
                        title = textwrap.shorten(title_element.strip(), width=100, placeholder="...")
                except Exception as title_e:
                    self.logger.debug(f"Could not fetch title for {url}: {title_e}")
                results.append({"url": url, "title": title})
                self.logger.debug(f"Found dork result: {title} - {url}")
            self.logger.info(f"Google dork search for '{dork_query}' complete. Found {len(results)} results.")
        except Exception as e_search: # Catch broad exceptions from the google search library
            ...
            if "HTTP Error 429" in str(e_search):
                self.logger.warning("Google Dork search likely blocked due to too many requests (HTTP 429). Try again later or use a proxy/VPN.")
    ...
    def bypass_bitly_url(self, bitly_url):
        ...
        try:
            ...
            response = requests.get(bitly_url, headers=DEFAULT_HEADERS, timeout=self.config.get("bitly_timeout", 10), allow_redirects=False)

            if response.status_code in (301, 302, 307, 308) and 'Location' in response.headers:
                original_url = response.headers['Location']
                self.logger.info(f"Bitly URL {bitly_url} bypassed. Original URL: {original_url}")
                return {"original_url": original_url, "bitly_url": bitly_url}
            else: # If no redirect, try parsing HTML (less common for bit.ly itself)
                response_html = requests.get(bitly_url, headers=DEFAULT_HEADERS, timeout=10, allow_redirects=True) # Allow redirects this time
                ...
                if meta_refresh and meta_refresh.get("content"):
                    url_match = re.search(r'url=[\'"]?([^\'" >]+)', meta_refresh["content"], re.IGNORECASE)
                    if url_match:
                        original_url = url_match.group(1)
                        self.logger.info(f"Bitly URL {bitly_url} bypassed (meta refresh). Original URL: {original_url}")
                        return {"original_url": original_url, "bitly_url": bitly_url}

                # Fallback for some structures if direct redirect fails
                links = soup.find_all("a", href=True)
                if links: # This is very generic
                    self.logger.debug(f"Trying to find original link in page content for {bitly_url} as direct redirect failed.")
                ...
                self.logger.warning(f"Could not bypass Bitly URL {bitly_url}. Status: {response.status_code}. Headers: {response.headers}")
                return {"error": "Could not resolve Bitly URL via redirect.", "status_code": response.status_code}
...
if __name__ == '__main__':
    test_config = {
        "log_level": "DEBUG",
        "VERIPHONE_API_KEY": os.environ.get("VERIPHONE_API_KEY_TEST") # Needs env var for full test
    }
    ...
    if dns_info: test_logger.info(f"DNS Info for google.com (first 100 chars):\n{dns_info[:100]}...")
    else: test_logger.error("Failed to get DNS info for google.com.")

    whois_info = net_utils.get_domain_info_hackertarget("google.com", "whois")
    if whois_info: test_logger.info(f"WHOIS Info for google.com (first 100 chars):\n{whois_info[:100]}...")
    else: test_logger.error("Failed to get WHOIS info for google.com.")
    ...
    if net_utils.veriphone_api_key:
        ...
        phone_data = net_utils.get_phone_info(os.environ.get("TEST_PHONE_NUMBER", "+12000000000")) # Use env var or placeholder
        if phone_data and not phone_data.get("error"):
            test_logger.info(f"Phone info: Country - {phone_data.get('country')}, Type - {phone_data.get('phone_type')}")
        else:
            test_logger.warning(f"Could not get phone info or error occurred: {phone_data}")
    else:
        test_logger.warning("Skipping Phone Info test as VERIPHONE_API_KEY is not set.")

    test_logger.info("\n--- Testing Google Dork Search ---")
    if google_search:
        dork_results = net_utils.google_dork_search("inurl:admin.php site:example.com", num_results=3)
        if dork_results and "results" in dork_results:
            ...
            test_logger.error("Google Dork search failed to return expected results structure.")
    else:
        test_logger.warning("Skipping Google Dork test as 'google' library is not installed.")
    ...
    if bypassed_info and "original_url" in bypassed_info:
        test_logger.info(f"Bypassed {test_bitly_url} -> {bypassed_info['original_url']}")
    else:
        test_logger.error(f"Failed to bypass Bitly URL {test_bitly_url}. Info: {bypassed_info}")
