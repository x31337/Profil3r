...
class UnifiedCliApp:
    ...
    def _load_config(self, config_file):
        try:
            ...
            path_options = [config_file, os.path.join(os.path.dirname(__file__), '..', config_file)]
            resolved_path = None
            ...
            if resolved_path:
                ...
                print(f"Warning: Config file '{config_file}' not found in standard locations. Using empty config.")
                return {}
    ...
    def _setup_logging(self):
        ...
        log_file = os.path.join(log_dir, self.config.get("cli_log_filename", 'unified_cli.log'))
    ...
    def fb_login_menu(self, fb_instance):
        choice = self._get_input("Login method? (1-Selenium UN/PW, 2-Cookies, 3-Load Session):", "1")
        if choice == "1":
            ...
            cookie_path = self._get_input("Cookie file path (leave blank for default/prompt):", fb_instance.cookie_file_path or None)
            if fb_instance.login_with_cookies(cookie_path if cookie_path else None): # Pass None if blank to trigger default/prompt
                self.logger.info("Facebook login with cookies successful (for requests session).")
            else:
                self.logger.error("Facebook login with cookies failed.")
        elif choice == "3":
            session_file = self.config.get("selenium_session_file", "facebook_selenium_session.json")
    ...
    def fb_actions_menu(self):
        ...
        with FacebookAutomation(config=self.config) as fb: # fb_instance is fb here
            ...
            action = self._get_input(
                "FB Action (post, like, comment, report, userinfo, friends, simulate, savesession, exit):", "exit"
            ).lower()

            if action == "post":
                content = self._get_input("Post content:", FacebookInteractionHelper.generate_post_text())
                if fb.create_post(content): self.logger.info("Post created.")
                else: self.logger.error("Failed to create post.")
                ...
                target = self._get_input("Post URL/Element context (optional, for specific post):")
                content = self._get_input("Comment text:", FacebookInteractionHelper.generate_comment_text())
                fb.comment_on_posts(post_urls_or_elements=[target] if target else None, comments_list=[content], max_comments=1)

            elif action == "report":
                url = self._get_input("Account URL to report:")
                reasons = self._get_input("Report reasons (comma-separated, e.g., 'Fake account,Harassment'):")
                reason_list = [r.strip() for r in reasons.split(',')] if reasons else None
                if url: fb.report_account(url, reason_keywords=reason_list)
            
            elif action == "userinfo":
                uid = self._get_input("User ID (default 'me'):", "me")
                fields = self._get_input("Fields (e.g., id,name,email):", "id,name,email")
                info = fb.get_user_info_graph_api(user_id=uid, fields=fields)
                if info: self.logger.info(f"User Info for {uid}: {json.dumps(info, indent=2)}")

            elif action == "friends":
                uid = self._get_input("User ID (default 'me'):", "me")
                limit = int(self._get_input("Limit:", "20"))
                friends = fb.get_friends_graph_api(user_id=uid, limit=limit)
                if friends is not None: self.logger.info(f"Friends of {uid} (limit {limit}): {json.dumps(friends, indent=2)}")
            ...
            else: self.logger.warning(f"Unknown Facebook action: {action}")

    # --- OSINT Utilities Actions ---
    def osint_actions_menu(self):
        self.logger.info("\n=== OSINT Utilities ===")
        action = self._get_input("OSINT Action (userrecon, emailfind, github, tempmail, exit):", "exit").lower()

        if action == "userrecon":
            uname = self._get_input("Username for reconnaissance:")
            if uname: results = self.osint_utils.user_reconnaissance(uname)
            if results: self.logger.info(f"User Recon Results:\n{json.dumps(results, indent=2)}")

        elif action == "emailfind":
            name = self._get_input("Full name to find emails for:")
            if name: results = self.osint_utils.find_emails_by_name(name)
            if results: self.logger.info(f"Email Finder Results:\n{json.dumps(results, indent=2)}")

        elif action == "github":
            uname = self._get_input("GitHub username:")
            if uname: info = self.osint_utils.get_github_user_info(uname)
            if info: self.logger.info(f"GitHub Info for {uname}:\n{json.dumps(info, indent=2)}")
            ...
            if email_info:
                self.logger.info(f"Temp mail generated: {email_info['email']}. Checking for messages...")
                self.wait_random(purpose="action_confirm") # Wait a bit for emails
                messages = self.osint_utils.get_temporary_emails(email_info)
                if messages: self.logger.info(f"Messages:\n{json.dumps(messages, indent=2)}")
                else: self.logger.info("No messages found for temp mail yet.")

        elif action == "exit": return
        else: self.logger.warning(f"Unknown OSINT action: {action}")

    # --- Network Utilities Actions ---
    def network_actions_menu(self):
        self.logger.info("\n=== Network Utilities ===")
        action = self._get_input("Network Action (iploc, domaininfo, phoneinfo, dork, bitly, exit):", "exit").lower()

        if action == "iploc":
            ip = self._get_input("IP Address (blank for own IP):")
            loc = self.network_utils.get_ip_location(ip or None)
            if loc: self.logger.info(f"IP Location:\n{json.dumps(loc, indent=2)}")
            ...
            if target: info = self.network_utils.get_domain_info_hackertarget(target, scan)
            if info: self.logger.info(f"HackerTarget '{scan}' for '{target}':\n{info}")

        elif action == "phoneinfo":
            num = self._get_input("Phone number (with country code):")
            if num: info = self.network_utils.get_phone_info(num)
            if info: self.logger.info(f"Phone Info:\n{json.dumps(info, indent=2)}")

        elif action == "dork":
            query = self._get_input("Google dork query:")
            if query: results = self.network_utils.google_dork_search(query)
            if results: self.logger.info(f"Google Dork Results:\n{json.dumps(results, indent=2)}")

        elif action == "bitly":
            url = self._get_input("Bitly URL to bypass:")
            if url: info = self.network_utils.bypass_bitly_url(url)
            if info: self.logger.info(f"Bitly Bypass:\n{json.dumps(info, indent=2)}")

        elif action == "exit": return
        else: self.logger.warning(f"Unknown Network action: {action}")
    ...
    def run_profil3r(self):
        ...
        if PROFIL3R_AVAILABLE:
            try:
                ...
                profil3r_instance = Profil3r(config_path=self.config.get("profil3r_config_path", "profil3r/config.json"))
                profil3r_instance.run() # Assuming it has a run method
                self.logger.info("Profil3r scan finished.")
            except Exception as e:
                self.logger.error(f"Failed to run Profil3r: {e}. Ensure Profil3r is correctly installed and configured.")
        else:
    ...
    def run_interactive(self):
        ...
        while True:
            ...
            if choice == '1': self.fb_actions_menu()
            elif choice == '2': self.osint_actions_menu()
            elif choice == '3': self.network_actions_menu()
            elif choice == '4': self.run_profil3r()
            elif choice == '5': self.logger.info("Exiting."); break
            else: self.logger.warning("Invalid option. Please try again.")

def main():
    # Basic argparse setup
    parser = argparse.ArgumentParser(description="Unified Automation and OSINT CLI.")
    parser.add_argument(
        "--config", default="config.json", help="Path to the main JSON configuration file."
    )
...
if __name__ == "__main__":
    ...
    if __package__ is None or __package__ == '':
        ...
        from modules.facebook_automation import FacebookAutomation, FacebookInteractionHelper
