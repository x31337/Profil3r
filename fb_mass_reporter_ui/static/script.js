document.addEventListener('DOMContentLoaded', function() {
    ...
    const reportingErrorDisplay = document.getElementById('reporting-error-display');
    ...
    addAccountBtn.addEventListener('click', () => {
        ...
        try {
            ...
            if (!Array.isArray(cookies) || !cookies.every(c => typeof c === 'object' && c.name && c.value)) {
                accountErrorDisplay.textContent = "Invalid cookie JSON format. Expected an array of objects with name and value properties.";
                return;
            }

            const c_user_cookie = cookies.find(c => c.name === 'c_user');
            const c_user_short = c_user_cookie ? String(c_user_cookie.value).substring(0, 6) : 'N/A';
            const alias = accountAliasInput.value.trim() || `Account (c_user: ${c_user_short}...)`;
            ...
            accountErrorDisplay.textContent = "Error parsing cookie JSON: " + e.message;
        }
    });
    ...
    evidenceFilesInput.addEventListener('change', () => {
        if (evidenceFilesInput.files.length > 0) {
            let fileNames = Array.from(evidenceFilesInput.files).map(f => f.name).join(', ');
    ...
    function updateStartButtonState() {
        ...
        startReportingBtn.disabled = !(anyAccountSelected && targetsProvided && justificationProvided);
    }
    // Initial call and on input changes
    [targetUrlsInput, justificationTextInput, accountListUl].forEach(el => {
        if(el === accountListUl) { // For account list, need to listen to changes on checkboxes
    ...
    startReportingBtn.addEventListener('click', async () => {
        const selectedAccountsData = accounts.filter((_, index) => {
            const checkbox = document.getElementById(`account-checkbox-${index}`);
            return checkbox && checkbox.checked;
        ...
        if (selectedAccountsData.length === 0) {
            reportingErrorDisplay.textContent = "Please select at least one account to use for reporting.";
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                addLogMessage("Backend process started successfully. Polling for status...");
                startStatusPolling();
            } else {
                reportingErrorDisplay.textContent = `Error starting backend process: ${result.message}`;
                addLogMessage(`Error starting backend process: ${result.message}`, 'ERROR');
    ...
    stopReportingBtn.addEventListener('click', async () => {
        if (!confirm("Are you sure you want to stop the current reporting process?")) return;
    ...
    function startStatusPolling() {
        ...
        statusInterval = setInterval(async () => {
            try {
                ...
                summaryProcessedSpan.textContent = status.summary.processed_targets || 0;
                summarySuccessfulSpan.textContent = status.summary.successful_reports || 0;
    ...
    function setReportingState(isReporting) {
        ...
        [cookieDataInput, accountAliasInput, addAccountBtn, clearAccountsBtn,
         targetUrlsInput, justificationTextInput, evidenceFilesInput].forEach(el => {
            el.disabled = isReporting;
        });
        // Disable account list checkboxes and remove buttons
        document.querySelectorAll('#account-list input[type="checkbox"], #account-list .remove-account-btn').forEach(el => {
            el.disabled = isReporting;
        });
    }
