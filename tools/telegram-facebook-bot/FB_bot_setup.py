...
def advanced_config():
    input(
        """There should be something here to configure the logging but I'm tired and this bot is waaay too complex to be such badly designed
...
def main():
    input(
        """Your FB Bot will now be set in the cuirrent folder, you will need:
...
Ready?
    ...
    ans = input(
        """Are you an advanced user who wants to customize logging?
Default is No
(y/n)
    """
    )
    ...
    ans = input(
        """Paste your bot's Telegram token
...
This input will not be checked
    ...
    input(
        """Now we will start building the pages.csv file that the Bot will use as a guide
...
"""
    ...
    while True:
        channel = input(
            """Paste here a @channelname you want to use the bot on
...
"""
        ...
        while True:
            page = input(
                "Now paste the link to the posts section of a FB page you want on that channel\n"
            )
            if page == "DONE":
                break
    ...
    ans = input(
        """paste now, if you want, a custom name for your .csv file
...
"""
    ...
    with open(csv_file, "w+", newline="", encoding="utf_8") as file:
        ...
        for row in lines:
            writer.writerow(row)
    ans = input(
        """How often you want the bot to wake up and check for new content?
...
"""
    ...
    ans = input(
        """Do you want to let people add pages to your bot via Telegram?
...
"""
    ...
    if ans.lower().startswith("n"):
        ...
        ans = input(
            """Paste now custom file to log all approved page additions
...
"""
        ...
        FB_adder_ini["LOG"]["log_file_name"] = (
            FB_adder_ini["LOG"]["log_file_name"] + "adder_"
        )
        FB_Bot_ini["ADDER"]["last_request_unix"] = "0"
        ans = input(
            """If you know your Telegram ID (not the @username, the actual numerical ID) paste it now
...
"""
        ...
        if ans.upper().strip().strip("'") == "NULL":
            msg_text = input(
                """Now we will find out your Telegram ID
Send a direcdft text message to the bot now and paste the same text here
"""
            )
            r = requests.get(
                TG_BASE.format(FB_Bot_ini["BASIC"]["bot_token"]) + "getUpdates"
            )
            if r.status_code != 200:
                print(
                    "The bot is unreachable from this computer, try again after cheking the token and the connection.\n I will crash now"
                )
                time.sleep(5)
                raise BaseException
            else:
                for update in json.loads(r.content):
                    if update["message"]["text"] == msg_text:
                        ans = input(
                            "Is your username: "
                            + str(update["message"]["from"]["username"] + "\n(y/n)")
                        )
                        if ans.lower().startswith("y"):
                            user_id = update["message"]["from"]["id"]
                            break
                    print(
                        "There might have been an error in parsing the bot's updates, this has caused a critical faliure.\nI will now crash"
                    )
                    time.sleep(5)
                    raise BaseException
                FB_adder_ini["BASIC"]["admin_id"] = str(user_id)
        ans = input(
            """Enter now the maximum number of pages to be added for a single user request
The default is 10
"""
        )
        if ans != "":
