...
def argument_parser():  # description of the program and command line arguments
    parser = argparse.ArgumentParser(
        description="Support script to add new FB pages and channels to running list"
    )
    parser.add_argument(
        "-ini_file",
        dest="ini_file",
        default="./FB_adder.ini",
        help="Path to INI file that sources all basic info to the bot, see the GitHub for more; defaults to ./FB_adder.ini",
    )
...
def config_parser(ini_file):
    ...
    if not isinstance(numeric_level, int):
        raise ValueError(
            "Invalid log level in configuration file: " + log_config["debug_level"]
        )
    log_config["log_file"] = (
        log_config["log_file_name"] + get_day(log_config["date_structure"]) + ".log"
    )
...
def start(bot, update):
	...
	Se sei interessato a come funziona il Bot, se vuoi modificarlo o attivare una tua versione vieni su [GitHub](https://github.com/MorenK1/telegram-facebook-bot)""",
...
def error(bot, update):
    bot.sendMessage(
        chat_id=update.message.chat_id,
        text="Your message was not recognized, please try again",
    )
...
def channel(bot, update, args, user_data):
    ...
    if len(args) != 1:
        bot.sendMessage(
            chat_id=update.message.chat_id,
            text="There was an error reading the name of the channel, they aren't supposed to have spaces\nThis is what the bot is reading: '"
            + str(args)
            + "'\nIf you followed our instructions correctly, you might want to contact the Developer",
        )
    try:
        channel_message = bot.sendMessage(
            chat_id=str(args[0]), text="TEST FOR THE CHANNEL"
        )
        time.sleep(5)
        bot.deleteMessage(
            chat_id=str(args[0]), message_id=channel_message["message_id"]
        )
        ...
        bot.sendMessage(
            ...
            text="Ok the channel "
            + str(args[0])
            + " seems to be working correctly, now send me the links to the posts sections of the Facebook pages you are interested in, one per line, with no spaces or anything\nMaximum numer of pages is "
            + str(max_n_of_pages)
            + ", exceeding pages will be ignored",
        )
        return PAGES
    except Exception as e:
        bot.sendMessage(
            chat_id=update.message.chat_id,
            text="There was an error trying to write on the channel, here's some detail:\n"
            + str(e)
            + "\nIf you followed our instructions correctly, you might want to contact the Developer",
        )
        logging.error(
            "There was an error trying to write on the channel, here's some detail:\n"
            + str(e)
        )
...
def pages(bot, update, user_data):
    ...
    pages = list(
        filter(lambda x: bool(re.search(r"https:\/\/www\.facebook\.com", x)), pages)
    )
    user_data["PAGES"] = []
    global max_n_of_pages
    ...
    bot.sendMessage(
        chat_id=update.message.chat_id,
        text="The pages were read correctly, here is what the bot found:\n"
        + " ".join(pages),
    )
    # if len(pages)==1:
...
def accept(bot, update):
    ...
    if str(update.message.from_user.id) == str(admin_id):
        ...
        with open(out_file, "a", encoding="utf_8") as file:
...
def see_all(bot, update):
    ...
    try:
        ...
        for page in pages:
            message = (
                message + str(page[0]) + " " + str(page[2]) + " " + str(page[4]) + "\n"
            )
        bot.sendMessage(chat_id=update.message.chat_id, text=message)
    except IOError:
        bot.sendMessage(
            chat_id=update.message.chat_id,
            text="The list is too long, iut needs to be split but I haven't implemented it yet",
        )
    ...
    try:
        ...
        if count == 0:
            bot.sendMessage(
                ...
                text="No pages with that name were found in the list",
            )
        else:
...
def main():
    ...
    con_handler = ConversationHandler(
        entry_points=[CommandHandler("add", add, pass_user_data=True)],
        states={
            CHANNEL: [
                CommandHandler("channel", channel, pass_args=True, pass_user_data=True)
            ],
            PAGES: [MessageHandler(Filters.text, pages, pass_user_data=True)],
        },
