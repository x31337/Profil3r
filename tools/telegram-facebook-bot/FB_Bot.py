...
def send_post(post):  # for text posts
    if (
        len(remove_tags(post["text"])) > 4000
    ):  # this SHOULD be recurively correct, splitting the text in n messages of 4000
        print("LOONG MESSAGE")
...
def find_photos(
    post,
):  # basically the same as find_photo but with different tags for multiple photo posts
    photos = []
    ...
    if len(video_areas) != 0 and video_areas is not None:
        ...
        if video_areas != [] and video_areas is not None:
            ...
            data_dict = json.loads(
                data
            )  # yes it's a JSON dictionary inside an HTML tag, I hate this timeline
            return data_dict["src"]
    return (
        -1
    )  # lives do have /videos/ in the URL but don't have a video area that I can use, to catch this possibility it returns -1
...
def content(
    ...
    if find_photo(post):
        ...
        if (
            post["video"] != -1
        ):  # if it's -1 then there was no video link and so it is send like a normal text post
...
def new_posts_handling(
    posts, last_time, bot, channel_ID, page_name
...
def generate_soup(
    URL,
):  # now we use RequestsHTML, which can compile Javascript and handes session errors better
    # with HTMLSession() as session:
    # retry = Retry(total=10, connect=3, redirect=5,
    # backoff_factor=0.5)  # i'm not even sure what it does but helps with facebook stoppoing the bot
    # adapter = HTTPAdapter(max_retries=retry)
    # session.mount('http://', adapter)
    # session.mount('https://', adapter)  # it's one of the reasons why I'm using RequestsHTML
    # r = session.get(URL)
    # patch_pyppeteer()
    # r.html.render()  # here the Javascript is rendered in a mock browser
    # body = r.html.find("body",
    # first=True).html  # and the body of the page is extracetd to be prcessed by BeautifulSoup
    ...
    try:
        while True:  # the main loop
            ...
            for page in pages:
                ...
                logging.info(
                    "SHOWN ON CHANNEL: "
                    + page_name.encode("ascii", "ignore").decode("ascii", "ignore")
                )
                URL = page[2]
                ...
                page[3] = new_posts_handling(
                    posts, last_time, TOKEN, channel_ID, page_name
                )
