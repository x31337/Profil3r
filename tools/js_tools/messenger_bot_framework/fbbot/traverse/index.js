...
Traverse.prototype.traverse = function (branch, payload, callback) {
  ...
  this.logger.debug({
    message: 'Traversing payload under branch handle',
    branch: branch,
    payload: payload
  });
  ...
  this.middleware(
    ...
    function (error, resolvedPayload) {
      ...
      if (error) {
        this.logger.error({
          message: 'middleware ran into an error',
          error: error,
          branch: branch,
          prefixed: this.prefixedBranch(branch),
          nextStep: nextStep,
      ...
      if (
        ...
        typeOf(resolvedPayload) != 'array'
      ) {
        this.logger.error({
          message: 'payload after middleware is not an Object or an Array',
          branch: branch,
          prefixed: this.prefixedBranch(branch),
          nextStep: nextStep,
        ...
        callback(
          new Error(
            'payload after <' +
              branch +
              '> middleware is not an Object or an Array'
          )
        );
        return;
      }
      ...
      this.logger.info({
        message: 'Successfully executed all middleware for current branch',
        branch: branch,
        prefixed: this.prefixedBranch(branch),
        payload: resolvedPayload
      });
      ...
      if (!nextStep || !(nextStep in resolvedPayload)) {
        this.logger.debug({
          message: 'no further steps possible, stop right here',
          branch: branch,
          prefixed: this.prefixedBranch(branch),
          nextStep: nextStep,
          payload: resolvedPayload
      ...
      nextStepPayload = this.linkParent(
        branch,
        resolvedPayload,
        resolvedPayload[nextStep]
      );
