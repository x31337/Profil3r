    ...
    tr = Traverse(steps, {
      middleware: function (branch, payload, cb) {
        t.deepEqual(
          ...
          'middleware payload, should match original object'
        );
...
tape('traverse - middleware error', function (t) {
  ...
  tr.traverse(original, function (err, result) {
    ...
    t.deepEqual(
      result,
      original,
      'expect unchanged result object with middleware error'
    );
  });
});
...
tape('traverse - broken middleware', function (t) {
  ...
  tr.traverse(original, function (err, result) {
    t.equal(
      ...
      'expect after middleware error on second iteration'
    );
    t.deepEqual(
      result,
      undefined,
      'expect no result object with after middleware error'
    );
  });
});
