...
function perRequest(fbbot, payloadType, request, t, callback) {
  ...
  fbbot.use(function(payload, cb) {
    t.deepEquals(
      payload,
      request.body,
      'global middleware should receive full payload, and it should not be modified at this point'
    );
    cb(null, payload);
  });
  ...
  fbbot.use(payloadType, function(payload, cb) {
    ...
    t.deepEquals(
      ...
      'middleware should receive ' +
        payloadType +
        ' object, augmented by built-in middleware'
    );
    t.deepEquals(
      payload.messaging,
      expected,
      payloadType + ' object has reference to the parent object, as prototyte'
    );
    t.notOk(
      payload.hasOwnProperty('messaging'),
      'parent property does not pollute ' + payloadType + ' object'
    );
    cb(null, payload);
  });
  ...
  fbbot.on(payloadType, function(payload, send) {
    ...
    t.equal(
      typeof send,
      'function',
      'expect send function for each message event'
    );

    t.deepEquals(
      payload,
      expected[payloadType],
      'event should receive ' + payloadType + ' object'
    );
    t.deepEquals(
      payload.messaging,
      expected,
      payloadType +
        ' event object has reference to the parent object, as prototyte'
    );
    t.notOk(
      payload.hasOwnProperty('messaging'),
      'parent property does not pollute ' + payloadType + ' event object'
    );
    t.deepEqual(
      payload,
      payload.messaging[payloadType],
      'keeps reference itself (kind of) through parent object'
    );

    if (
      payloadType == 'message' &&
      expected[payloadType].type == 'quick_reply'
    ) {
      t.equal(
        typeof payload.quick_reply.payload,
        'object',
        'should parse quick_reply payload into an object'
      );
    }
    ...
    if (payloadType == 'postback') {
      t.equal(
        ...
        'object',
        'should parse postback payload into an object'
      );
    }
  });
...
function perMessage(fbbot, payloadType, envelop, t) {
  (payloads[payloadType]['middleware'] =
    payloads[payloadType]['middleware'] || []).push(envelop);
  (payloads[payloadType]['event'] = payloads[payloadType]['event'] || []).push(
    envelop
  );
      ...
      fbbot.on('message.' + envelop.message.type, function(message) {
        ...
        t.deepEquals(
          ...
          'message.' +
            ...
            ' event should receive message object'
        );
      });
    }
