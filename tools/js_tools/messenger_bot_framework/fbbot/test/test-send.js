...
common.iterateSendings(function(sending, handle, callback) {
  ...
  test.test(
    ...
    function(t) {
      ...
      t.plan(
        (sending.error ? 3 : 6) +
          (sending.expectedTests || 0) +
          (sending.count ? sending.count.expected : 0)
      );
      ...
      common.startApiServer(
        function(request, respond) {
          t.equal(
            ...
            'should supply proper access token'
          );
          t.deepEqual(
            request.body,
            sending.expected,
            'expects to have proper payload for message type: ' + type
          );
        ...
        function(fbbotOptions, done) {
          ...
          t.equal(
            fbbot.options.apiUrl,
            fbbotOptions.apiUrl + fbbotOptions.pageAccessToken,
            'respect custom apiUrl and augment it with correct token'
          );
          ...
          if (sending.count) {
            fbbot.on(sending.count.hook, function(payload) {
              t.equal(
                typeof payload,
                'object',
                'expect event ' + sending.count.hook + ' payload to be object'
              );
            });
          }
          ...
          fbbot.on('send.message', function(data) {
            if (expectedId) {
              t.equal(
                ...
                expectedId,
                'should have recipient id available via linked parent object'
              );
            } else {
              t.equal(
                data.parent.recipient['phone_number'],
                expectedPhone,
                'should have recipient phone_number available via linked parent object'
              );
            }
          });
            ...
            args.concat(function(error, result) {
              if (sending.error) {
                t.ok(
                  ...
                  'expect to error with message: ' + sending.error
                );
              } else {
              ...
              t.deepEqual(
                result,
                sending.response,
                'expect to pass response all the way through'
              );
