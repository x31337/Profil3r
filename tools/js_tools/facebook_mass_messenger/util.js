module.exports = {
  formatMessage: (message, variables, rng, recursion = 0) => {
    ...
    for (let i = 0; i < message.length; ++i) {
      ...
      if (message[i] === '{') {
        if (escape()) {
          ++level;
          if (!spintaxStart) lastOption = spintaxStart = i + 1;
        }
      } else if (message[i] === '|') {
        if (level === 1 && escape()) {
        ...
        if (spintaxStart && escape()) {
          ...
          if (!level) {
            options.push(message.substr(lastOption, i - lastOption));
            const option = module.exports.formatMessage(
            ...
            message =
              message.substr(0, spintaxStart - 1) +
              option +
              message.substr(i + 1);
        ...
        if (!level && escape()) {
          if (variableStart) {
            ...
            if (name in variables)
              value = module.exports.formatMessage(
            ...
            message =
              message.substr(0, variableStart - 1) +
              value +
              message.substr(i + 1);

            i = variableStart + value.length - 2;
            variableStart = 0;
          } else variableStart = i + 1;
        }
      }
    }
  ...
  getFriendVariables: (req, users, values = true) => {
    ...
    if (req.messageIDInvalid) return friendvariables;

    const all = req.query.all || 'true';
    if (req.session.uid) {
      const user = users.findOne({ id: req.session.uid });
      if (user) {
        friendvariables.nname = {
          value: user.nicknames[req.query.id],
          readonly: false
        };
          ...
          if (friend) {
            for (const variable in module.exports.friendVariableMap) {
              if (module.exports.friendVariableMap.hasOwnProperty(variable))
                friendvariables[variable] = {
                  value: friend[module.exports.friendVariableMap[variable]],
                  readonly: true
                };
            }
          }
    ...
    if (req.session.variables) {
      for (const variable in req.session.variables[req.messageID]) {
        if (req.session.variables[req.messageID].hasOwnProperty(variable))
          friendvariables[variable] = {
            value: values ? req.session.variables[req.messageID][variable] : '',
            readonly: false
          };
      }
    }
    ...
    if (
      ...
      req.query.id in req.session.friendvariables[req.messageID]
    ) {
      for (const variable in req.session.friendvariables[req.messageID][
        req.query.id
      ]) {
        if (
          req.session.friendvariables[req.messageID][
          ...
          if (
            variable !== 'nname' &&
            !(variable in module.exports.friendVariableMap) &&
            variable in friendvariables
          ) {
            if (
              (values &&
                req.session.friendvariables[req.messageID][req.query.id][
                  variable
                ].length) ||
              !values
            )
            ...
            delete req.session.friendvariables[req.messageID][req.query.id][
              variable
            ];
        }
      }
