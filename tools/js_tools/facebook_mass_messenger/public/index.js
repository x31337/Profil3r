  ...
  item
    ...
    .on('input', function () {
      ...
      post(
        '/setfriendvariable',
        {
          id: selecteditem.children('.circle-check').data('id'),
          name: name,
          value: $(this).val()
        },
        function () {
    ...
    .removeAttr('id')
    .removeAttr(readonly ? 'placeholder' : '')
      ...
      function (result) {
        if (result.error) {
          ...
          modal
            .find('.modal-body')
            .text(result.error)
            .append('<br><br>Logging in and out might fix the issue.');
          modal.modal();
        } else location.href = '/';
      },
      'json'
