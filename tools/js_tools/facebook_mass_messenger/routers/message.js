...
module.exports = (app, users) => {
  ...
  app.get('/getmessage', (req, res) => {
    ...
    req.session.messages = req.session.messages.filter((value, index) => {
      ...
      if (
        ...
        index < req.session.variables.length &&
        Object.keys(req.session.variables[index]).length
      )
        return true;

      if (
        req.session.friends &&
        index < req.session.friends.length &&
        req.session.friends[index].length
      )
        return true;
  ...
  app.post('/sendmessage', async(req, res) => {
    ...
    if (
      !req.session.friends ||
      !req.session.friends.length ||
      !req.session.friends[req.messageID].length
    )
