...
module.exports = (app, users) => {
  app.post('/setfriend', (req, res) => {
    ...
    if (!req.session.friends) req.session.friends = [];

    if (!req.session.friends[req.messageID])
      req.session.friends[req.messageID] = [];

    if (!req.body.ids) req.body.ids = [req.body.id];
  ...
  app.post('/setfriendvariable', (req, res) => {
    ...
    if (!req.session.friendvariables) req.session.friendvariables = [];
    ...
    req.session.friendvariables[req.messageID][req.body.id][req.body.name] =
      req.body.value;
  ...
  app.post('/setnickname', (req, res) => {
    ...
    if (!user.friends.includes(req.body.id)) return res.end();
    ...
    if (req.body.nickname) {
      ...
      if (friend) {
        ...
        user.nicknames[req.body.id] = friend.alternateName
          ? friend.alternateName
          : friend.firstName;
