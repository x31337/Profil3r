...
request(
  {
    url: 'https://raw.githubusercontent.com/FFace32/facebook-mass-message/master/package.json',
    json: true
  },
  (error, response, body) => {
    if (!error && response.statusCode === 200) {
      if (require('./package.json').version !== body.version)
        console.log(
          `Version ${body.version} available! See https://github.com/FFace32/facebook-mass-message`
        );
    }
    console.log();
    ...
    app.use((req, res, next) => {
      if (req.body.message_id) req.messageID = parseInt(req.body.message_id);
      else req.messageID = parseInt(req.query.message_id);

      if (isNaN(req.messageID)) req.messageID = -1;

      req.messageIDInvalid =
        req.messageID < 0 ||
        !req.session.messages ||
        req.messageID > req.session.messages.length;

      next();
    });

    let users;
    const db = new loki('user-store.db', {
      env: 'NODEJS',
      autosave: true,
