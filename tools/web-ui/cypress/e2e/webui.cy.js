...
describe('Facebook Mass Reporter UI', () => {
  ...
  it('should successfully submit a report without evidence and update history', () => {
    ...
    const justification =
      'This is a test justification for Cypress: no evidence.';
    ...
    cy.wait('@getReports');
    cy.get('#report-history-container table tbody tr').should(
      'have.length.at.least',
      1
    );
    cy.get('#report-history-container table tbody tr td').should(
      'contain',
    ...
    cy.get('#report-history-container table tbody tr td.status-success').should(
      'contain.text',
      'success'
    );
  });
  ...
  it('should successfully submit a report with an evidence file', () => {
    ...
    const justification =
      'This is a test justification for Cypress: with evidence.';
        ...
        results: [
          {
            email: 'test@example.com',
            success: true,
            message: 'Mocked success with evidence'
          }
        ],
        report_id: 124
      }
  ...
  it('should require Target URL and Justification fields (HTML5 validation)', () => {
    ...
    cy.get('#target-url').type(
      `https://www.facebook.com/test_no_justification_${Date.now()}`
    );
    cy.get('button[type="submit"]').click();
