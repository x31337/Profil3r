...
describe('Facebook Mass Reporter UI', () => {
    ...
    describe('Account Management', () => {
        it('should allow adding a valid account', () => {
            cy.get('#cookie-data').type(validCookieJsonString, { parseSpecialCharSequences: false, delay: 0 });
            ...
            cy.get('#account-list ul li').first().should('contain', 'c_user: 123456...');
        });
        ...
        it('should show an alert for invalid cookie JSON', () => {
            ...
            cy.get('#cookie-data').type('{"invalid": "json"}', { parseSpecialCharSequences: false, delay: 0 });
            cy.get('#add-account-btn').click().then(() => {
                expect(alertStub).to.be.calledOnce;
                // Check if the alert message contains expected text (flexible check)
                expect(alertStub.getCall(0).args[0]).to.contain('Invalid cookie JSON format');
            });
        ...
        it('should allow clearing all accounts', () => {
            ...
            cy.get('#cookie-data').clear().type(validCookieJsonString.replace("123456789", "987654321"), { parseSpecialCharSequences: false, delay: 0 });
    ...
    describe('Reporting Process Flow (with mocked backend)', () => {
        beforeEach(() => {
            ...
            cy.intercept('GET', '/api/status', (req) => {
                ...
                req.reply({
                    active: true,
                    logs: [`[${new Date().toLocaleTimeString()}] [INFO] Backend process started...`, `[${new Date().toLocaleTimeString()}] [INFO] Processing target 1...`],
                    summary: {
                        total_targets: 1,
                        processed_targets: 0,
        ...
        it('should start reporting, show logs, and allow stopping', () => {
            ...
            cy.get('#log-output').should('contain', 'Backend process started successfully.');
            ...
            cy.intercept('GET', '/api/status', {
                active: false,
                logs: [`[${new Date().toLocaleTimeString()}] [INFO] Backend process started...`, `[${new Date().toLocaleTimeString()}] [INFO] Processing target 1...`, `[${new Date().toLocaleTimeString()}] [INFO] Reporting finished.`],
                summary: {
                    total_targets: 2,
                    processed_targets: 2,
                    ...
                    failed_reports: 1,
                    details: [
                        {account_alias: accountAlias, target_url: "http://facebook.com/someuser", status: "Success", message: "Reported"},
                        {account_alias: accountAlias, target_url: "http://facebook.com/anotheruser", status: "Failure", message: "Failed"}
                    ]
                }
            }).as('getStatusFinished');

            cy.wait('@getStatusFinished'); // Wait for the "finished" status
             cy.get('#log-output').should('contain', 'Reporting process finished or stopped.'); // From JS
        ...
        it('should allow stopping an in-progress report', () => {
            ...
            cy.intercept('GET', '/api/status', {
                ...
                logs: [`[${new Date().toLocaleTimeString()}] [INFO] Stop request sent. Process will halt soon.`, `[${new Date().toLocaleTimeString()}] [INFO] Reporting stopped by user.`],
                summary: { /* ... potentially incomplete summary ... */
                    total_targets: 2, processed_targets: 1, successful_reports: 0, failed_reports: 1, details: [{account_alias: accountAlias, target_url: "http://facebook.com/someuser", status: "Failure", message: "Stopped by user"}]
                }
            }).as('getStatusStopped');

            cy.wait('@getStatusStopped');
            cy.get('#log-output').should('contain', 'Reporting process finished or stopped.');
            cy.get('#start-reporting-btn').should('not.be.disabled');
            cy.get('#stop-reporting-btn').should('not.be.visible');
        });
    });
