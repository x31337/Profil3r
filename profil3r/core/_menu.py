...
def menu(self):
    ...
    for separator, value in separators.items():
        # Separator title
        separators_menu[0]["choices"].append(
            Separator("{} - Exemple : john{}doe".format(separator, value))
        )
        # Separator
    ...
    modules_list = sorted(
        [
            module
            for module in self.CONFIG["plateform"]
            if self.CONFIG["plateform"][module]["enabled"] == "yes"
        ]
    )
    ...
    categories = sorted(
        list(
            set(
                [
                    content["type"]
                    for module, content in self.CONFIG["plateform"].items()
    ...
    services_menu = [
        {
            ...
            "validate": lambda answer: (
                "You must choose at least one service !" if len(answer) == 0 else True
            ),
        }
    ]
